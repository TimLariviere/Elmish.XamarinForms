...
module MauiProgram =
    ...
    let attachWith (element: IFrameworkElement) (arg: 'arg) (definition: RunnerDefinition<'arg, 'msg, 'model, 'externalMsg>) =
        let runner = Runner() :> IRunner<'arg, 'msg, 'model, 'externalMsg>
        runner.Start(definition, arg)
        runner.AttachView(element, ValueNone)
        runner

    let attach (element: IFrameworkElement) (definition: RunnerDefinition<unit, 'msg, 'model, 'externalMsg>) =
        attachWith element () definition

    let runWith (arg: 'arg)(definition: RunnerDefinition<'arg, 'msg, 'model, 'externalMsg>) =
        let runner = Runner() :> IRunner<'arg, 'msg, 'model, 'externalMsg>
        runner.Start(definition, arg)
        let target = runner.CreateView(ValueNone)
        (runner, target)

    let run (definition: RunnerDefinition<unit, 'msg, 'model, 'externalMsg>) =
        runWith () definition