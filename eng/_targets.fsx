...
let nugetVersion =
    match Environment.environVarOrDefault "GITHUB_RUN_NUMBER" "" with
    | builderNumber when not (System.String.IsNullOrWhiteSpace builderNumber) ->
        if release.NugetVersion.Contains("-") then
            let x = release.NugetVersion.Split('-')
            sprintf "%s.%s-%s" x.[0] builderNumber x.[1]
        else
            sprintf "%s.%s" release.NugetVersion builderNumber
    | _ -> release.NugetVersion

printfn "NuGet Version = %s" nugetVersion
...
Target.create "CreateGitHubRelease" (fun _ ->
    ...
    try
        ...
        |> GitHub.draftNewRelease repositoryOwner repositoryName nugetVersion release.SemVer.PreRelease.IsSome (release.Notes |> List.map (sprintf "- %s"))
